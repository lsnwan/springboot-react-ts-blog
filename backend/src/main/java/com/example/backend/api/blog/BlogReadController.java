...
public class BlogReadController {
    ...
    @PostMapping("/{blogId}")
    public ResponseEntity<?> getBlogContent(@PathVariable String blogId, @RequestBody @Valid CommonDto.Request request) {
        ...
        List<BlogContentDto> blogContent = blogService.getBlogContent(blogId.substring(1), request.getPageIndex(), request.getPageUnit());
        log.info(blogContent.toString());

        List<BlogContentRegisteredDto> blogRegisteredCalendar = blogService.getBlogRegisteredCalendar(blogId.substring(1));
